...
def test(ligand, ensemble):
    ##test
    test_T5 = np.load('./multi-feature/' + '{}'.format(ligand) + '/ProtT5_test.npy').reshape(-1,1,1024)
    test_bio_vec = np.load('./multi-feature/' + '{}'.format(ligand) + '/bio_test.npy').reshape(-1,1,75)
    test_NABert = np.load('./multi-feature/' + '{}'.format(ligand) + '/NABert_test.npy').reshape(-1,1,1024)
