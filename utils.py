...
def load_data_vec(ligand):
    train_gen = np.load('./multi-feature/' + '{}'.format(ligand) + '/gene_train.npy').reshape(-1, 1, 1024)
    train_bio_vec = np.load('./multi-feature/' + '{}'.format(ligand) + '/bio_train.npy').reshape(-1, 1, 75)
    train_dyna = np.load('./multi-feature/' + '{}'.format(ligand) + '/dyna_train.npy').reshape(-1, 1, 1024)
